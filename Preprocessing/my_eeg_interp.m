...
% $Log: eeg_interp.m,v $
% Revision 2  2009/07/18 18:46:30  Juan E Kamienkowski
% Repaired bug in (original) line 185 with dimesions. Now is working only
% for 'invdist' method.
%
...
function EEG = my_eeg_interp(ORIEEG, bad_elec, method)
...
% revision 2
            [Xi,Yi,badchansdata(:,t)] = griddata(ygood, xgood , tmpdata(goodchans,t)',...
                                                    ybad, xbad, method); % interpolate data                                            
        end
        fprintf('\n');
    end;
...
%     tmpdata(goodchans, :,:) = EEG.data;
% revision 2
    tmpdata = zeros(length(goodchans) + length(badchans), EEG.pnts, EEG.trials);
    tmpdata(goodchans, :,:) = EEG.data(goodchans,:,:);
